...
def post_to_slack(message: str = None, channel_id: str | None = None, blocks: list = None) -> bool:
    """
    Slack にメッセージを送信する。
    ...
    Args:
        message: 送信するメッセージ文字列（blocks指定時は省略可）
        channel_id: 送信先チャンネルID（省略時は Webhook でデフォルトチャネルに送信）
        blocks: Block Kit フォーマットのブロック配列
    ...
    if channel_id:
        ...
        try:
            ...
            if result.get("ok"):
                print(f"[INFO] Slack 通知送信 (channel={channel_id})")
                return True
            else:
